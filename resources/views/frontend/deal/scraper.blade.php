<div class="row">
    <div class="col-lg-7">
        <div class="row mb-50">
            <div class="col-lg-6">
                <input wire:model.defer="url" type="text" placeholder="Enter URL">
                @error('url')
                <x-admin.alert type="danger" :message="$message" />
                @enderror
                <button wire:click="scrape" class="btn  btn-md" name="login">Start Here</button>
            </div>
        </div>
        <div class="row">
            ...
            {!! Form::open(['route' => 'deal.store', 'method' => 'post']) !!}
                ...
                <div class="row">
                    <div class="form-group col-lg-6">
                        {!! Form::text('title', (isset($title) ? $title : null), ['class' => 'form-control', 'placeholder' => 'Deal Title']) !!}
                        @error('title')
                        <x-admin.alert type="danger" :message="$message" />
                        @enderror
                    </div>
                    <div class="form-group col-lg-6">
                        {!! Form::text('link', (isset($link) ? $link : null), ['class' => 'form-control', 'placeholder' => 'Deal Link']) !!}
                        @error('link')
                        <x-admin.alert type="danger" :message="$message" />
                        @enderror
                    </div>
